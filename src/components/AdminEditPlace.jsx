...
const AdminEditPlace = ({ place, onClose, onSaved }) => {
  ...
  const handleSave = async () => {
    ...
    if (isStaticPlace) {
      alert('Built-in places cannot be edited via the admin panel.');
      return;
    }
    setLoading(true);
    try {
      ...
      await updateDoc(doc(db, 'approvedPlaces', place.docId), {
        ...form,
        image: imageUrl,
    ...
    <div className="modal-overlay">
      <div className="modal-content">
        ...
        <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {isStaticPlace ? (
            <div style={{ background: '#fef3c7', borderRadius: '8px', padding: '12px', fontSize: '13px', color: '#92400e' }}>
              ⚠️ This is a built-in place and cannot be edited here.
            </div>
          ) : (
            <div style={{ background: '#f0fdf4', borderRadius: '8px', padding: '12px', fontSize: '13px', color: '#166534' }}>
              🔑 Admin edit — changes go live immediately, no approval needed.
            </div>
          )}
          ...
          <button
            onClick={handleSave}
            disabled={loading || isStaticPlace}
            className="booking-submit-btn"
            style={{ opacity: isStaticPlace ? 0.5 : 1 }}
          >
